...
def generate_visualization_for_cluster(docs, event2mention, temporal_orders, cluster, cluster_nb, base_output_path):
    ...
    generate_visualization(docs, event2mention, topo_sort, temporal_orders, join(join(base_output_path, 'visualizations_ordered'), 'cluster_{}_event_coref.html'.format(cluster_nb)))
...
if __name__ == "__main__":
    ...
    temporal_orders = set()
    with open(join(args.coref_dir, 'temporal.cs'), 'r') as f:
        for line in f:
            es = line.strip().split('\t')
            if 'temporal_before' in es[1].lower():
                temporal_orders.add((es[0], es[2]))
            elif 'temporal_after' in es[1].lower():
                temporal_orders.add((es[2], es[0]))

    # generate_visualization_for_cluster
    for cluster_nb, cluster in enumerate(clusters):
        generate_visualization_for_cluster(docs, event2mention, temporal_orders, cluster, cluster_nb, args.coref_dir)
